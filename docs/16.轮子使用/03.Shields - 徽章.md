...
再 Markdown 里，直接使用徽章生成的 url 地址并不会生成徽章，它仅仅只是一个 url，生成徽章的格式：
...
3. 把这个 `url` 粘贴到上图的红色框里，然后会请求这个 `url`，获得 `label` 为<mark>可乐</mark>，`message` 为<mark>Kbt</mark>。如图：
...
可以看出参数很多。它相比较端点徽章的特点是多了个请求参数，也就是 `query`，不需要整个 url 的内容。点击 `data type` 下拉菜单，知道返回的内容支持 `json`、`xml`、`yaml`。
