...
- 首先下载能登录 VPN 的软件：Easy Connect，如何下载呢？

    打开文档，三个 VPN 任选一个，在浏览器访问该 VPN，然后提示下载 Easy Connect 软件到本地，它默认安装在 C 盘，只能通过注册表修改默认安装目录，这里建议 C 盘大的就不折腾了。

- 下载软件后，打开软件，刚才三个 VPN 任选一个，然后登录
    ...
    初始化后，需要输入用户名，密码。两个都在文档里，就叫用户名和密码。然后成功登录 VPN。
...
- 使用 **IE 浏览器** 访问：<https://10.10.9.11>，记得先登录 VPN 才能访问该网站
