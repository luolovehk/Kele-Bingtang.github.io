...
- 第一个 location（第 4 行代码）：当客户端访问 `http://localhost/index.html`，两个 `proxy_pass` 效果是一样的
...
![image-20211128121825920](https://cdn.jsdelivr.net/gh/Kele-Bingtang/static/img/Nginx/20211128121827.png)

![image-20211128121700067](https://cdn.jsdelivr.net/gh/Kele-Bingtang/static/img/Nginx/20211128121701.png)

![image-20211128122210286](https://cdn.jsdelivr.net/gh/Kele-Bingtang/static/img/Nginx/20211128122211.png)
...
<code-group>
  ...
  server {
      ...
      ssl_certificate      /root/cert/server.cert; # 上一步生成的 cert 证书路径，根据需求修改
      ssl_certificate_key  /root/cert/server.key; # 上一步生成的 key 证书路径，根据需求修改
...
如果直接输入 `www.youngkbt.com`，会默认加上『 http:// 』 而不是『 https:// 』，如何修改为『 https:// 』呢，我们利用 Rewrite 功能

```nginx {3,7}
server {
    listen 80;
    server_name www.youngkbt.com;   # 如果是 www.youngkbt.com 发送请求
    
    location / {
        # ......
        rewrite ^(.*) https://www.youngkbt.com$1;  # 则改为 https 方式
        # ......
	}
    # ......
}
