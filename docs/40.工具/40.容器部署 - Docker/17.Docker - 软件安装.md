...
- 运行 MySQL 服务
    ...
    ```sh
    docker run -d --name mysql -e MYSQL_ROOT_PASSWORD=123456 -p 3307:3306 mysql:tag
    ```

    使用 3307 是因为宿主机可能已经安装了 MySQL，它占用了 3306 端口。

- 将容器数据位置与宿主机位置挂载保证数据安全

- ```sh
    docker run -d --name mysql -v /root/mysql/data:/var/lib/mysql -v /root/mysql/conf.d:/etc/mysql/conf.d -e MYSQL_ROOT_PASSWORD=123456 -p 3307:3306 mysql:tag
    ```

    宿主机挂载目录和 conf.d 文件目录根据需求决定，包括密码。
