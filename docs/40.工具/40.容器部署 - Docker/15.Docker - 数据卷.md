...
其次 **具体目录挂载** 虽然可以指定宿主机的某个路径，但是一旦挂载目录多了起来，因为目录过于分散导致无法集中管理。值得注意的是：第一次启动容器挂载时，必须确保容器的挂载目录数据备份好，因为该挂载方式会将 **宿主机的挂载目录内容覆盖掉容器的挂载目录内容**。所以，有数据则备份，再挂载。

最推荐的是 **默认目录挂载**，docker 指定了一个默认目录，无论挂载目录有多少，都集中在默认目录下管理。它和具体目录挂载的另一个区别是：第一次启动容器挂载时，**容器的挂载目录内容会覆盖宿主机的挂载目录内容**。
