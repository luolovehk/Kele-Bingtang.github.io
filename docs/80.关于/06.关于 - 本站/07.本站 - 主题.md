...
- 获取项目后，首先建议先把 `.github/workflows/baiduPush.yml` 去掉，这是百度推送相关的，部署时不填入自己的百度信息会报错，后期需要再加入。打开 `docs/.vuepress/config` 目录下的 `themeConfig.js` 文件，滑到最下方，把 `htmlModules` 注释掉，这是首页右下方的广告：二维码广告
