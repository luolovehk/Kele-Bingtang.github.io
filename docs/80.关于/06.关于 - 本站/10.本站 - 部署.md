...
- GitHub Pages 完全免费，不需要个人购买服务器、域名。你完全可以
...
- 初始化项目根目录
    ...
    **修改提交地址**
    ...
    进入 `.git`目录后，打开 `config` 文件夹，手动添入信息：`url`、`fetch`
...
自此，如果可以在仓库 `master` 下看到下面的源码，`gh-pages` 下看到打包后的文件，代表成功了
...
自动化部署利用了 `GitHub Actions` ，仅需实现过程1：推送项目源码到 `master`，即可自动实现过程2
...
3. 开启 `Github Pages` 和 `Gitee Pages`，两者都有自己的部署地址，前者网速满，后者网速块
...
多个仓库都可以配置一个公钥，而某一仓库前这些往多个仓库，则需要私钥当作通行证和方向盘。
...
刚开始我只部署到 `GitHub Page`，但是毕竟是国外，网速满，所以就考虑部署到 `Gitee`，但是又不舍得 `GitHub`，于是重心依然放在 `GitHub`，只需要同步到 `Gitee Pages` 即可。其实我访问博客，一直访问的是 `Gitee Pages` 提供的地址。还是比较快。
