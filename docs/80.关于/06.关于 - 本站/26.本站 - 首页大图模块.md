...
如果你另外需要添加所有页面的背景图，则打开 docs/.vuepress/config.js（新版是 config.ts）文件，在 `themeConfig` 中添加如下代码：
...
多条描述，如果填写则覆盖 config.js 的 description，不填写则默认读取 config.js 的 description，**descFadeIn 为 true 生效**。
...
**提示：里面的图片路径我已经配置好，获取都是 public 根目录下的 fantasy 文件夹下的图片，如果你更改了路径，则下方代码也要改路径。**
...
此时你是没有导航栏的透明效果、标题的淡入淡出效果，这些效果我都放在了大图配置模块里，如果你没有配好大图模块，则从头阅读步骤开始，如果已经配好大图模块，则
...
仅仅 `Fantasy` 组件会将图片覆盖 `IndexBigImg` 组件的图片，而其他的样式都会保留。
