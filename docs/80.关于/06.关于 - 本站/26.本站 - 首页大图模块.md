...
此时我相信你是直接从我的首页进入该文章，而不是通过地址栏直达此处，当然，如果你是直达，可以浏览一番本内容介绍的大图模块，[点击跳转](/)。(PS：不用跳转了，参考下方图片即可，我已经换全背景图了😂🤣)



![](https://cdn.jsdelivr.net/gh/Kele-Bingtang/static/img/关于/关于本站/20220114232335.png)

如果你打算看实际的 demo，则跳转我的首页，然后 F12，找到 class 为 `fantasy` 的元素，将其隐藏掉，就能看到效果。

![image-20220115000056859](https://cdn.jsdelivr.net/gh/Kele-Bingtang/static/img/%E5%85%B3%E4%BA%8E/%E5%85%B3%E4%BA%8E%E6%9C%AC%E7%AB%99/20220115000058.png)
...
此时仅仅是开启首页的大图效果，下滑或跳转新的页面后，依然是灰白色背景，如果你想 **首页大图 + 全屏背景图**，则注释掉上方的 `bannerBg`，打开 docs/.vuepress/config.js（新版是 config.ts）文件，在 `themeConfig` 中添加如下代码：
...
indexImg: {
  ...
  desc: ["Web前端技术博客，积跬步以至千里，致敬每个爱学习的你", "故事由我书写，旅程由你见证，传奇由她聆听", "这一生波澜壮阔或是不惊都没问题"],  // 多个描述，如果填写则覆盖 config.js 的 description，不填写默认读取 config.js 的 description，descFadeIn 为 true 生效
...
描述的淡出效果持续时间，**descFadeIn 为 true 生效**，默认 100 毫秒。
...
当有多个 desc 时，一个 desc 展示完后或准备开始时，多少时间后出现下一个 desc，默认 800 毫秒。
...
上面的所有配置是属于大图效果，如果你喜欢我的首页背景图：全背景、颜色随时间变化、日历显示当天、时间同步当前、播放音乐有音谱 ...（可能时间久远，我也许换背景图了🤣）
...
**提示：代码块里的图片路径我已经配置好，获取都是 public 根目录下的 fantasy 文件夹下的图片，如果你更改了路径，则下方代码块也要改路径。**
...
此时你是没有导航栏的透明效果、标题下 desc 的淡入淡出效果，这些效果我都放在了大图配置模块里，如果你没有配好大图模块，则从头阅读步骤开始，如果已经配好大图模块，则
...
仅仅 `Fantasy` 组件的图片会覆盖 `IndexBigImg` 组件的图片，而其他的样式和功能都会保留。
