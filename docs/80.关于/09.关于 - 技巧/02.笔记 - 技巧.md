...
`Voding` 官方说该组件仅适用于放置代码块，放其他内容在体验上并不友好。经过测试，我放入文字也是可以的。

而且 &lt;code-block> 放在 <code-group&gt; 里时，首行缩进两个空格的 tab，经过测试，四个空格的 tab 出问题。

`Voding` 官方推荐使用 [vuepress-plugin-tabs](https://superbiger.github.io/vuepress-plugin-tabs) 插件，但是我尝试下载了很多次，报错，我安装了报错需要的依赖，也还是报错，索性就没使用插件。
